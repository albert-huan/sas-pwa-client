...
const dict = {
  zh: {
    ...
    "opt.keep.d": "注入 display-mode / navigator.standalone 伪装，并按下方间隔派发活动脉冲，避免 SAS 空闲超时掉线（改动后需重新打开该站点的窗口才生效）",
    ...
    "list.open": "打开",
    "list.newWindow": "新窗口",
    "list.closeWindows": "关闭窗口",
    "list.windowCount": "{n} 个窗口",
    "list.edit": "编辑",
    ...
    "toast.windowsClosed": "已彻底关闭 {n} 个窗口",
    "toast.closeWinFail": "关闭窗口失败：{e}",
    ...
    "cred.savedBothPlain": "已保存用户名「{user}」和密码：当前平台没有 Windows DPAPI，密码只做了十六进制编码就存进配置目录的 credentials.json，等同于明文 —— 请确保这台机器上的其他用户读不到该文件。",
    "cred.savedUser": "已保存用户名「{user}」，未保存密码。",
    "cred.none": "未保存登录凭据。填写用户名和密码后点保存即可（密码加密存放、不回显；密码框留空表示不改动已存密码）。",
    "cred.nonePlain": "未保存登录凭据。填写用户名和密码后点保存即可（本平台没有 DPAPI，密码仅做十六进制编码存放、等同于明文，且不回显；密码框留空表示不改动已存密码）。",
    "toast.credSaved": "登录凭据已加密保存",
    "toast.credSavedPlain": "登录凭据已保存（本平台无 DPAPI，等同明文存放）",
    "toast.credFail": "凭据保存失败：{e}",
    "toast.savedReopenNeeded": "配置已保存；保活设置有变化，已打开的 {n} 个窗口需重新打开才生效",
    "btn.checkUpdate": "检查更新",
    "update.latest": "已是最新版本（v{current}）",
    "update.available": "发现新版本 v{latest}（当前 v{current}）。是否用浏览器打开下载页？",
    "update.opened": "已在浏览器中打开下载页",
    "update.checkFail": "检查更新失败：{e}；若仓库为私有或网络不通，可手动到 Release 页查看。",
    ...
    "advanced.heading": "高级",
    "opt.devtools.t": "允许打开开发者工具（F12）",
    "opt.devtools.d": "站点窗口跑的是远程 SAS 页面：开启后，能碰到这个窗口的人可以通过 DevTools 读到注入脚本里用于自动填充的登录凭据。建议只在排查页面问题时临时开启，且只对新打开的窗口生效。",
    "confirm.deleteWithWindows": "确定删除「{name}」？它还有 {n} 个窗口正在运行，删除后这些窗口会被一并彻底关闭。",
  },
  en: {
    ...
    "opt.keep.d": "Injects display-mode / navigator.standalone spoof and emits an activity pulse at the interval below to prevent SAS idle timeout. Changes take effect after reopening the site window.",
    ...
    "list.closeWindows": "Close windows",
    "list.windowCount": "{n} window(s)",
    "list.edit": "Edit",
    ...
    "toast.windowsClosed": "Closed {n} window(s)",
    "toast.closeWinFail": "Failed to close window: {e}",
    ...
    "cred.savedBothPlain": "Saved username “{user}” and password. This platform has no Windows DPAPI: the password is only hex-encoded into credentials.json under the config directory, which is effectively plaintext — make sure other users on this machine cannot read that file.",
    ...
    "cred.nonePlain": "No login credentials saved. Fill in the username and password and save (no DPAPI on this platform, so the password is only hex-encoded — effectively plaintext — and never shown; leaving the password blank keeps the saved one unchanged).",
    "toast.credSaved": "Login credentials encrypted and saved",
    "toast.credSavedPlain": "Login credentials saved (no DPAPI here — stored effectively as plaintext)",
    "toast.credFail": "Credential save failed: {e}",
    "toast.savedReopenNeeded": "Configuration saved; keep-alive settings changed — reopen the {n} open window(s) to apply",
    "btn.checkUpdate": "Check for updates",
    "update.latest": "You are on the latest version (v{current})",
    "update.available": "New version v{latest} available (current: v{current}). Open the download page in your browser?",
    "update.opened": "Download page opened in your browser",
    "update.checkFail": "Update check failed: {e}; if the repo is private or the network is blocked, check the Releases page manually.",
    ...
    "opt.devtools.t": "Allow opening DevTools (F12)",
    "opt.devtools.d": "Site windows load the remote SAS page: once this is on, anyone with access to that window can read the login credential injected for auto-fill from DevTools. Keep it off except while troubleshooting; it only affects windows opened afterwards.",
    "confirm.deleteWithWindows": "Delete “{name}”? It still has {n} running window(s), which will be closed along with it.",
  },
};
